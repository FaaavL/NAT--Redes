...
def run():
    ...
    net.get('r').cmd('echo 1 > /proc/sys/net/ipv4/ip_forward &')
    net.get('r').cmd('iptables -t nat -A POSTROUTING -o r-eth0 -j MASQUERADE &')
    net.get('r').cmd('iptables -A FORWARD -i r-eth0 -o r-eth1 -m state --state RELATED,ESTABLISHED -j ACCEPT &')
